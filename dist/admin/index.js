...
const courseSchema = zod_1.z.object({
    ...
    subject: zod_1.z.string().min(2).default("Mathematics"),
    medium: zod_1.z.string().min(2).default("English"),
    tutor_name: zod_1.z.string().min(2),
    thumbnail_url: zod_1.z.string().url().optional(),
    original_price: zod_1.z.coerce.number().nonnegative(),
    discount_price: zod_1.z.coerce.number().nonnegative(),
    is_free: zod_1.z.boolean().default(false),
    chapters: zod_1.z.array(zod_1.z.any()).default([]),
    videos: zod_1.z.array(zod_1.z.string()).default([]),
    pdfs: zod_1.z.array(zod_1.z.string()).default([]),
    images: zod_1.z.array(zod_1.z.string()).default([]),
    notes: zod_1.z.array(zod_1.z.string()).default([]),
    duration: zod_1.z.string().default(""),
    students_enrolled: zod_1.z.coerce.number().int().nonnegative().default(0),
...
const normalizeCoursePayload = (body) => ({
    ...
    tutor_name: pickFirst(body.tutor_name, body.tutorName, body.teacher_name, body.teacherName, body.instructor_name, body.instructorName),
    thumbnail_url: pickFirst(body.thumbnail_url, body.thumbnail, body.image, body.image_url, body.banner, body.banner_url),
...
const createCourseHandler = async (req, res) => {
    try {
        const payload = courseSchema.parse(normalizeCoursePayload(req.body));
        ...
        try {
            await supabaseQuery_1.supabaseQuery.insert("courses", {
                id: ref.id,
                title: payload.title,
...
app.post("/courses", admin_1.adminMiddleware, createCourseHandler);
app.post("/courses/add", admin_1.adminMiddleware, createCourseHandler);
